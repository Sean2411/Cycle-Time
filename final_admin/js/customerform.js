...
function canSubmit() {
  return isLetterValid($customer_name) && isLetterValid($customer_city) && isLetterValid($customer_state) && isLetterValid($customer_country) && isnumericValid($customer_zip) && isEmailValid() &&  isPhoneValid() && isFaxValid() && isnumericValid($customer_company) && isalphanumericValid($customer_address);
}
