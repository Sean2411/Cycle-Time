// only for dev - remove for release
window.log = function(){
...
(function($){
	...
	var validationMessages = {
		typeMismatch: function(elem){
			// getAttribute() gives us 'real' type (HTML5 types)
			switch (getType(elem)) {
				case 'color':
				...
				switch (getType(elem)) {
					...
					case 'select-multiple':
						...
						if (elem.is('[multiple]')) {
							...
							// Browsers default select-ones to 1st option so they
							// will always be selected.
							// return (selected_count == 1);
							
							// for now, just assume option w/ blank value
							// until we have more info on this
							if (elem.val() === '') {
								return false;
							}
						}
	...
	$.widget('ui.webfield', {
		...
		_create: function(){
			...
			self.element.addClass('ui-field ui-corner-all').bind('attrchange.webfield', function(event){
				...
				// also changing an attribute may mean changing constraints so we re-validate
				...
				// TODO: do we need event attributes?
			});
			
			self.element.filter('input, textarea').bind('change.webfield keyup.webfield', function(){
				self.checkValidity();
			});
			
			//self.element.filter('input[type=number]').bind('keyup.webfield', function(evt){
			//	log(self.element.val().replace('', ''));
			//});
			
			self.element.filter('select').bind('change.webfield', function(){
				self.checkValidity();
			});
